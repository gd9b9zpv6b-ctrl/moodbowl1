-- Add let_flow as a releasing choice.
-- wash stays valid so historical diary rows keep reading.

alter table public.diaries drop constraint if exists diaries_bowl_release_check;

alter table public.diaries
  add constraint diaries_bowl_release_check
  check (
    bowl_release is null
    or bowl_release in (
      'empty',
      'set_aside',
      'send_away',
      'wash',
      'let_flow',
      'keep_hug'
    )
  );

comment on column public.diaries.bowl_release is
  'Symbolic release action · empty|set_aside|send_away|wash|let_flow|keep_hug';
